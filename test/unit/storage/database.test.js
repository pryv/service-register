...
describe('Redis Database', () => {
  describe('#deleteUser(username)', () => {
    describe('user data should be deleted', () => {
      ...
      before(async () => {
        ...
        await usersStorage.updateFields(
          username,
          {
            email: [
              {
        ...
        assert.isTrue(
          await redisExists(`${info.randomField}:randomField`),
          `before the tests, ${info.randomField}:randomfield exists`
        );
        assert.isTrue(
          await redisExists(`${info.email}:email`),
          `before the tests, ${info.email}:email exists`
        );
        ...
        assert.isTrue(
          keys.length > 0,
          `before the tests, ${username}:${db.INACTIVE_FOLDER_NAME} exists`
        );
      });
      it('[55G5] deletes the user', async () => {
        await db.deleteUser(info.username);
        assert.isFalse(
          await redisExists(`${info.username}:users`),
          'user info is gone'
        );
        assert.isFalse(
          await redisExists(`${info.username}:server`),
          'user server is gone'
        );
      });
      ...
      it('[777Y] deletes user non-active unique fields', async () => {
        ...
        assert.isFalse(
          await redisExists(`${inactiveEmailValue}:email`),
          'inactive email link is gone'
        );
        assert.isTrue(
          keys.length === 0,
          `after deletion, ${info.username}:${db.INACTIVE_FOLDER_NAME} doesn't exist`
        );
      });
    });
  });
  describe('#setServerAndInfos', () => {
    describe('when given user "foobar"', () => {
      ...
      it('stores user information', async () => {
        assert.isTrue(await redisExists('foobar:users'));
        const storedInfo = await bluebird.fromCallback((cb) =>
          redis.hgetall('foobar:users', cb)
        );
        assert.strictEqual(storedInfo.username, 'foobar');
        assert.strictEqual(storedInfo.email, 'a@b.ch');
      });
      it('stores email index', async () => {
        const emailIndex = await bluebird.fromCallback((cb) =>
          redis.get('a@b.ch:email', cb)
        );
        assert.strictEqual(emailIndex, 'foobar');
      });
      it('stores server name', async () => {
        const serverName = await bluebird.fromCallback((cb) =>
          redis.get('foobar:server', cb)
        );
    ...
    it('lower cases email when storing it in redis (key)', async () => {
      ...
      assert.isTrue(await redisExists('a@b.ch:email'));
    });
  ...
  describe('#emailExists(email, cb)', () => {
    ...
    it('is case insensitive for email', async () => {
      assert.isTrue(
        await bluebird.fromCallback((cb) => db.emailExists('A@B.ch', cb))
      );
    });
  });
  describe('#getUIDFromMail(email, cb)', () => {
    ...
    it('is case insensitive for email', async () => {
      const uid = await bluebird.fromCallback((cb) =>
        db.getUIDFromMail('a@B.CH', cb)
      );
