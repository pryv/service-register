...
nconf.defaults({
  ...
  net: {
    // manly used in /network/dataservers
    aahostings: {},
...
/**
 * @typedef {Array<ServerConfig>} ServerList
 */

/**
 * @typedef {OldServerDefinition | ServerDefinition} ServerConfig
 */
...
function translateConfiguration() {
  ...
  if (customDnsEntries != null) {
    nconf.set(
      'dns:staticDataInDomain',
      Object.assign({}, staticDataInDomain, customDnsEntries)
    );
  }
  // Handle hostings
...
function validateConfiguration() {
  ...
  if (mxEntries == null || !Array.isArray(mxEntries)) {
    throw parseError(
      'Expecting "dns:mail" to be an array (even empty) of MX entries.'
    );
  }
      ...
      if (typeof token !== 'string') {
        throw parseError(
          'invitationToken "' +
            ...
            '" at position ' +
            i +
            ' in the "invitationTokens" array is not a string.'
        );
      }
      if (token.length < 5) {
        throw parseError(
