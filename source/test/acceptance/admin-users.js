...
describe('GET /admin/servers/:srcServerName/rename/:dstServerName', function(){
  ...
  for (var key = 0; key < tests.length; key++) { // create PATH and method
    tests[key].it = tests[key].desc + ', src: ' + tests[key].srcServerName+' dest:'+ tests[key].srcServerName;
    tests[key].url = '/admin/servers/'+ tests[key].srcServerName+'/rename/'+ tests[key].dstServerName;
    tests[key].method = 'GET';
